...
Name | Type | Required | Description
-----|---------|-----|---------|
app | string | yes | The name of the learncube app. Must be virtualclassroom. Recordings and homework apps are coming soon |
...
userid | string | yes | This is the id of the participant that is entering the classroom. Each user must have a unique id. If your api is in testing mode, we get a random user from an api to fill in the data. In production, the data will be filled in on your side. |
userType | string | no | Optional user type to overwite any settings for an already created class. |
validateUrl | url | no | Optional url to do the validation on your server, without making a call to the Learncube API authentication service. |
